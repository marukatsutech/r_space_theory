# ==========================================
# Photon model (rotation vector pair model)
# ==========================================
...
# ==========================================
class OrbitBase:
    def __init__(self, ax, origin=np.zeros(3), radius0=1., radius1=0., arrow_length0=1., arrow_length1=0,
                 phase=0.,
                 line_style0='-', line_style1='-', line_width0=1, line_width1=1, color0='gray', color1='gray'):
        ...
        self.arrow_length0 = arrow_length0
        self.arrow_length1 = arrow_length1

        self.phase = phase

        self.line_style0 = line_style0
        self.line_style1 = line_style1
        self.line_width0 = line_width0
        ...
        # --- Quiver ---
        self.quiver_obj0 = None
        self.quiver_obj1 = None

        # --- Guide elements (Circles and Phase line) ---
        # Ensure these are initialized after the bases exist
        self.plt_circle0, = self.ax.plot([], [], [], lw=self.line_width0, color=self.color0, ls=self.line_style0,
                                         alpha=1.)
        self.plt_circle1, = self.ax.plot([], [], [], lw=self.line_width1, color=self.color1, ls=self.line_style1,
                                         alpha=1.)
        self.plt_phase_line0a, = self.ax.plot([], [], [], lw=1., ls="--", color=color0)
        self.plt_phase_line0b, = self.ax.plot([], [], [], lw=1., ls="--", color=color0)
        self.plt_marker0a, = self.ax.plot([], [], [], marker="o", ms=5, color=color0)
        self.plt_marker0b, = self.ax.plot([], [], [], marker="o", ms=5, color=color0)
    ...
    def update_diagrams(self):
        # --- Update Quiver (The Arrow) ---
        if self.quiver_obj0:
            self.quiver_obj0.remove()

        self.quiver_obj0 = self.ax.quiver(
             self.origin[0], self.origin[1], self.origin[2],
             self._basis_z[0], self._basis_z[1], self._basis_z[2],
             length=self.arrow_length0, color=self.color0, linewidth=self.line_width0,
             arrow_length_ratio=0.2, normalize=False, ls=self.line_style0
        )
        ...
        # Circle points on the x-y local plane
        c_pts = (np.cos(theta)[:, None] * self._basis_x +
                 np.sin(theta)[:, None] * self._basis_y) * self.radius0 + self.origin
        self.plt_circle0.set_data_3d(c_pts[:, 0], c_pts[:, 1], c_pts[:, 2])

        # --- Update Phase Marker ---
        p_vec0a = (np.cos(self.phase) * self._basis_x +
                   np.sin(self.phase) * self._basis_y) * self.radius0 + self.origin
        self.plt_phase_line0a.set_data_3d([self.origin[0], p_vec0a[0]],
                                          [self.origin[1], p_vec0a[1]],
                                          [self.origin[2], p_vec0a[2]])
        self.plt_marker0a.set_data_3d([p_vec0a[0]], [p_vec0a[1]], [p_vec0a[2]])

        p_vec0b = (-np.cos(self.phase) * self._basis_x +
                   -np.sin(self.phase) * self._basis_y) * self.radius0 + self.origin
        self.plt_phase_line0b.set_data_3d([self.origin[0], p_vec0b[0]],
                                          [self.origin[1], p_vec0b[1]],
                                          [self.origin[2], p_vec0b[2]])
        self.plt_marker0b.set_data_3d([p_vec0b[0]], [p_vec0b[1]], [p_vec0b[2]])
    ...
    def set_arrow_length0(self, length):
        self.arrow_length0 = length
        self.update_diagrams()

    def set_arrow_length1(self, length):
        self.arrow_length1 = length
        self.update_diagrams()

    def get_phase_point0a(self):
        p_vec0a = (np.cos(self.phase) * self._basis_x +
                   np.sin(self.phase) * self._basis_y) * self.radius0 + self.origin
        return p_vec0a

    def get_phase_point0b(self):
        p_vec0b = (-np.cos(self.phase) * self._basis_x +
                   -np.sin(self.phase) * self._basis_y) * self.radius0 + self.origin
        return p_vec0b
...
class PhotonApp:
    def __init__(self, root):
        self.title = "Photon model (rotation vector pair model)"
        self.root = root
        ...
        # --- CREATE ORBIT BASE ---
        self.orbit_base0 = OrbitBase(self.ax, radius0=self.orbit_radius_init)
        self.orbit_base0.apply_rotation((np.pi/4.), np.array([1., 0., 0.]))
        self.orbit_base0.apply_rotation((np.pi / 4.), np.array([0., 0., 1.]))

        self.orbit_base1 = OrbitBase(self.ax, radius0=self.orbit_radius_init)
        self.orbit_base1.apply_rotation((np.pi / 2.), np.array([1., 0., 0.]))
        self.orbit_base1.apply_rotation((np.pi / 4.), np.array([0., 0., 1.]))
        ...
        label_orbit_r0 = ttk.Label(self.btn_frame, text="Orbit Radius_0", style="BigFont.TLabel")
        label_orbit_r0.pack(side='left')
        var_orbit_r0 = tk.StringVar(self.btn_frame)
        var_orbit_r0.set(str(self.orbit_radius_init))

        sb_orbit_r0 = tk.Spinbox(
            self.btn_frame, textvariable=var_orbit_r0, format="%.2f", from_=0.01, to=1., increment=0.01,
            command=lambda: self.set_orbit_radius0(float(var_orbit_r0.get())), width=5, font=("Arial", 24)
        )
        sb_orbit_r0.pack(side='left')

        label_orbit_r1 = ttk.Label(self.btn_frame, text="Orbit Radius_1", style="BigFont.TLabel")
        label_orbit_r1.pack(side='left')
        var_orbit_r1 = tk.StringVar(self.btn_frame)
        var_orbit_r1.set(str(self.orbit_radius_init))

        sb_orbit_r1 = tk.Spinbox(
            self.btn_frame, textvariable=var_orbit_r1, format="%.2f", from_=0.01, to=1., increment=0.01,
            command=lambda: self.set_orbit_radius1(float(var_orbit_r1.get())), width=5, font=("Arial", 24)
        )
        sb_orbit_r1.pack(side='left')
    ...
    def set_orbit_radius1(self, radius):
        self.orbit_base1.set_radius0(radius)
        self.update_orbit_location()

    def set_orbit_radius0(self, radius):
        self.orbit_base0.set_radius0(radius)
        self.update_orbit_location()
    ...
    def update_orbit_location(self):
        orbit_point_0a = self.orbit_base0.get_phase_point0a()
        orbit_point_0b = self.orbit_base0.get_phase_point0b()

        self.orbit_base1.set_origin(orbit_point_0a)
        self.rotation_vector_blue.set_origin(orbit_point_0b)

        orbit_point_1a = self.orbit_base1.get_phase_point0a()
        orbit_point_1b = self.orbit_base1.get_phase_point0b()

        self.rotation_vector_red.set_origin(orbit_point_1a)
        self.rotation_vector_green.set_origin(orbit_point_1b)

    def update_diagrams_with_phase(self, phase):
        self.orbit_base0.set_phase(phase)
        self.orbit_base1.set_phase(phase)
    ...
    def loop(self, frame):
        if self.is_playing:
            ...
            # --- UPDATE DIAGRAMS ---
            phase = np.pi * self.frame_count / 180.
